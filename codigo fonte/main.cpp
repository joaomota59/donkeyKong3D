...
#define BLOCOS 46 //quantidade de blocos da fase(por onde o personagem vai andar)
#include <string>
...
tBloco buracos[3]; //vetor que indica os buracos do cenário
...
void timer_callback (int value) {
	...
	//verifica se houve colisão com algum barril
	for (int i = 0; i < qte_barris; i++) {
		//se colidiu com um barril, restaura os valores
		if (colisao (barris[i].x, barris[i].y, barris[i].z, barris[i].raio, personX, personY, personZ, raioPerson) ) {
			...
			gameover = true;
			qte_barris = 0;
			PlaySound (TEXT ("../audios/perdeu.wav"), NULL, SND_SYNC); //muda o som do fundo
			break;
		}

		//verificações para evitar o personagem sair da tela
		if (barris[i].x <= -1) {
		...
		int flag = 0, number = -1; //variáveis de controle
	...
	for (int i = 0; i < 3; i++) { //buracos
		//verifica se o personagem colidiu com um buraco
		if (colisaoEP (buracos[i].x, buracos[i].y, buracos[i].z, buracos[i].raio * 2 + 0.25, 0.06, 0.08, personX, personY, personZ, raioPerson) )
			personZ += 0.035; // se colidiu, transala-o para baixo
	}

	//verifica se colidiu com a princesa
	if (colisao (0.23, 0.0, -1.21, 0.07, personX, personY, personZ, raioPerson) ) {
		printf ("Ganhou dd");
		PlaySound (TEXT ("../audios/ganhou.wav"), NULL, SND_SYNC); //Se colidiu, troca a música e sai do jogo
		exit (0);
	}
...
void keyboard_special (int key, int x, int y) {
	...
	case GLUT_KEY_UP://seta cima
		...
		if ( !pulo ) { //verifica se o personagem não pulou
			...
			if (flag1 == 0) //se não colide mais com a escada
				cima = false;
		}
...
void display (void) {
	...
	if (menu) //verifica se o usuário não clicou em start
		menuPrincipal();
	else { //se não clicou
		if (gameover) { //se colidiu com o barril
			menuGameOver();
		} else { //se não colidiu com o barril
			criaCenario();
			criaPersonagens();
		}
	}
	...
	glutSwapBuffers();
	intervalo += 1; //incrementa a quantidade de vezes que o display foi chamado
}
...
void criaCenario() { //quantidade de blocos do cenario
	...
	int auxiliar = 0; //variável para saber quantos blocos já foram adicionados
	tBloco b; //novo bloco
	...
	for (int k = 0; k < 5; k++) { //numero de andares 'k'
		...
		for (int i = 0; i < BLOCKS; i++, auxiliar++) { //numero de blocos em cada andar 'i'
			...
			if (k == 0 || (k < 4 && i < 11) || (k == 4 && i > 3 && i < 6) ) {
				...
				if (k == 0) {
					if (i == 0) //se for o primeiro bloco do primeiro andar, ele não colide
						b.colide = false;
					else
						b.colide = true;
				} else if (k == 1) {
					if (i == BLOCKS - 2 || i == BLOCKS - 3) //se for o penúltimo o ou antepenultimo bloco do segundo andar, não colide
						b.colide = false;
					else
						b.colide = true;
				} else if (k == 2) {
					if (i < 2) //se for o primeiro ou o segundo bloco do terceiro andar, não colide
						b.colide = false;
					else
						b.colide = true;
				}
					...
					if (i == BLOCKS - 2 || i == BLOCKS - 3) //se for o penúltimo o ou antepenultimo bloco do quarto andar, não colide
						b.colide = false;
					else
						b.colide = true;
				}

				blocks[auxiliar] = b; //adciona o bloco no vetor de blocos.
			} else { //adiciona o buraco de cada andar no vetor de buracos
				if (k == 1) {
					b.x = 1.0;
					b.y = 0.0;
					b.z = 0.5;
			...
			glTranslatef (tx, ty, tz); //translada o cubo
			criaCubo (0.09); //cria o cubo do bloco em questão
	...
	criaEscada (0.59, 0.29, 0.0); //cria as escadas
	...
	//se o display foi executado 200 vezes desde a ultima criação de barril e a quantidade de barris é menor que 6
	if (intervalo % 200 == 0 && qte_barris < 6) {
		srand (time (NULL) );
		tBarril b; //cria novo barril
		b.x = 0.2;
		b.y = 0.0;
		b.z = -0.65;
		b.raio = 0.05;
		b.velX = velocidadesBarril[rand() % 3]; //sorteio da velocidade
		b.velZ = 0.015;
		b.rotacao = -0.02;
		b.esquerda = true;
		b.direita = false;
		b.baixo = false;
		barris[qte_barris++] = b; //adiciona no vetor de barris
	}
	...
	for (int i = 0; i < qte_barris; i ++) {
		...
		if (barris[i].esquerda)
			barris[i].x -= barris[i].velX; //se ele caminha para a esquerda, translado de forma negativa

		if (barris[i].direita)
			barris[i].x += barris[i].velX; //se for pra direita, translado de forma positiva

		if (barris[i].baixo) {
			barris[i].z += barris[i].velZ; //se for pra baixo, translado de forma positiva
			barris[i].baixo = false; 
		}

		barris[i].rotacao = - barris[i].rotacao;  //constante em qualquer situa??o para dar a impressão do barril rolar
	...
	//personagem
	glPushMatrix();
	glColor3f (0.0f, 0.0f, 1.0f); //seto a cor azul
	glTranslatef (personX, personY, personZ); //translado
	glutSolidSphere (raioPerson, 30, 30); //crio uma esfera
	glPopMatrix();
